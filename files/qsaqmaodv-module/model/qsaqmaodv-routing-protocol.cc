 ...
 *
 * Based on
 *      NS-2 QSAQMAODV model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      QSAQMAODV-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/QSAQMAODV-UU
 *
...
#include "qsaqmaodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("QsaqmaodvRoutingProtocol");

namespace qsaqmaodv
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for QSAQMAODV control traffic
const uint32_t RoutingProtocol::QSAQMAODV_PORT = 654;

/**
 * \ingroup qsaqmaodv
 * \brief Tag used by QSAQMAODV implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::qsaqmaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Qsaqmaodv")
...
{
    static TypeId tid =
        TypeId("ns3::qsaqmaodv::RoutingProtocol")
            ...
            .AddAttribute("RewardW3", "Reward weight for Energy_residual",
                          ...
                          MakeDoubleChecker<double>())
    .AddAttribute("RewardW4","Queue-state weight",DoubleValue(0.20),MakeDoubleAccessor(&RoutingProtocol::m_qsW4),MakeDoubleChecker<double>(0.0,1.0))
            ...
            .AddAttribute("SeqNoWindow", "Window length for Δ_Seq counting",
                          ...
                          MakeTimeChecker())
                .AddAttribute("QueueHighThreshold","HIGH_LOAD entry",DoubleValue(0.70),MakeDoubleAccessor(&RoutingProtocol::m_queueHighThreshold),MakeDoubleChecker<double>(0.0,1.0))
    .AddAttribute("QueueLowThreshold","HIGH_LOAD exit",DoubleValue(0.30),MakeDoubleAccessor(&RoutingProtocol::m_queueLowThreshold),MakeDoubleChecker<double>(0.0,1.0))
...
RoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    *stream->GetStream() << "Node: " << m_ipv4->GetObject<Node>()->GetId()
                         ...
                         << ", QSAQMAODV Routing table" << std::endl;
...
{
  // QSAQMAODV: push initial params + start adaptive controller
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No qsaqmaodv interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
    ...
    {
        // QSAQMAODV: ε-greedy selection over primary + SA-Q-learned alternates.
        RoutingTableEntry chosenRt = rt;
        m_qtable.SelectEpsilonGreedy(rt, chosenRt, &m_routingTable);
        // QSAQMAODV-FIX-V2: per-packet SA-Q-update with neighbour-freshness reward.
        {
            ...
            m_qtable.UpdateQValueOrCreate(chosenRt, ack, delayS, eFrac,
                                   std::min(1.0, (double)chosenRt.GetHop() / 10.0));
        }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // QSAQMAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == QSAQMAODV_PORT)
                    {
                        // QSAQMAODV packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("QSAQMAODV does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvQsaqmaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), QSAQMAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvQsaqmaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), QSAQMAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("QSAQMAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in QSAQMAODV operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when QSAQMAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // QSAQMAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available QSAQMAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid QSAQMAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by qsaqmaodv
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(QSAQMAODVTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, QSAQMAODV_PORT));
}
...
void
RoutingProtocol::RecvQsaqmaodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("QSAQMAODV node " << this << " received a QSAQMAODV packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("QSAQMAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    {
    case QSAQMAODVTYPE_RREQ: {
        RecvRequest(packet, receiver, sender);
        break;
    }
    case QSAQMAODVTYPE_RREP: {
        RecvReply(packet, receiver, sender);
        break;
    }
    case QSAQMAODVTYPE_RERR: {
        RecvError(packet, sender);
        break;
    }
    case QSAQMAODVTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    {
        // QSAQMAODV: store alternate reverse route from duplicate RREQ (multipath capability).
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(QSAQMAODVTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), QSAQMAODV_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(QSAQMAODVTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), QSAQMAODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(QSAQMAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, QSAQMAODV_PORT));
}
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    // QSAQMAODV: SA-Q-update on RREP + SeqNo tracking.
    {
        ...
        m_qtable.UpdateQValueOrCreate(newEntry, /*ack=*/1.0, /*delaySec=*/0.005, eFrac,
                       std::min(1.0, (double)newEntry.GetHop() / 10.0));
    }
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(QSAQMAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(QSAQMAODVTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, QSAQMAODV_PORT));
        }
    }
}
...
{
    uint32_t cur=m_queue.GetSize(),cap=m_queue.GetMaxQueueLen();
    return cap==0?0.0:std::min(1.0,(double)cur/(double)cap);
}
