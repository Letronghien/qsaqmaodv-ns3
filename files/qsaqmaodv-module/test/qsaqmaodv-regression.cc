...
#include "qsaqmaodv-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/qsaqmaodv-helper.h"
 ...
 *
 * \brief QSAQMAODV regression test suite
 */
class QsaqmaodvRegressionTestSuite : public TestSuite
{
  public:
    QsaqmaodvRegressionTestSuite()
        : TestSuite("routing-qsaqmaodv-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("qsaqmaodv-chain-regression-test"), TestCase::QUICK);
...
} g_qsaqmaodvRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("QSAQMAODV chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & QSAQMAODV
    QsaqmaodvHelper qsaqmaodv; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(qsaqmaodv);
    ...
    streamsUsed += qsaqmaodv.AssignStreams(*m_nodes, streamsUsed);
    // QSAQMAODV uses m_size more streams
