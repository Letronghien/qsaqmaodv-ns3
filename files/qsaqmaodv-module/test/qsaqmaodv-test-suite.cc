 ...
 */
#include "ns3/qsaqmaodv-neighbor.h"
#include "ns3/qsaqmaodv-packet.h"
#include "ns3/qsaqmaodv-rqueue.h"
#include "ns3/qsaqmaodv-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("QSAQMAODV TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(QSAQMAODVTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), QSAQMAODVTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(QSAQMAODVTYPE_RREP);
 ...
 * \ingroup qsaqmaodv-test
 *
 * \brief Unit test for QSAQMAODV routing table entry
 */
...
struct QsaqmaodvRqueueTest : public TestCase
{
    QsaqmaodvRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
QsaqmaodvRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&QsaqmaodvRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&QsaqmaodvRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &QsaqmaodvRqueueTest::CheckTimeout, this);
...
void
QsaqmaodvRqueueTest::CheckSizeLimit()
{
...
void
QsaqmaodvRqueueTest::CheckTimeout()
{
 ...
 * \brief Unit test for QSAQMAODV routing table entry
 */
struct QsaqmaodvRtableEntryTest : public TestCase
{
    QsaqmaodvRtableEntryTest()
 ...
 * \ingroup qsaqmaodv-test
 *
 * \brief Unit test for QSAQMAODV routing table
 */
struct QsaqmaodvRtableTest : public TestCase
{
    QsaqmaodvRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup qsaqmaodv-test
 *
 * \brief QSAQMAODV test suite
 */
class QsaqmaodvTestSuite : public TestSuite
{
  public:
    QsaqmaodvTestSuite()
        : TestSuite("routing-qsaqmaodv", UNIT)
    {
        ...
        AddTestCase(new QsaqmaodvRqueueTest, TestCase::QUICK);
        AddTestCase(new QsaqmaodvRtableEntryTest, TestCase::QUICK);
        AddTestCase(new QsaqmaodvRtableTest, TestCase::QUICK);
    }
} g_qsaqmaodvTestSuite; ///< the test suite
